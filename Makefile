...
build: cmd/*/*.go internal/*/*.go pkg/*/*.go scripts/version.sh Makefile vendor api
	@echo "compile"
	@go build -ldflags "-X 'main.AppVersion=`sh scripts/version.sh`'" cmd/gogrpc/main.go && \
	mkdir -p build/gogrpc/bin && mv main build/gogrpc/bin/gogrpc && \
	mkdir -p build/gogrpc/configs && cp configs/gogrpc/* build/gogrpc/configs && \
	mkdir -p build/gogrpc/log
